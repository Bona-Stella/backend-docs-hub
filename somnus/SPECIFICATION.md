...
### B. Wake(Walk) 모드 (기상 기준 → 취침 추천)
> "이때 일어나려면 언제 잠들어야 할까?" (UI 텍스트는 Walk로 표기)
...
## 6. 개발자 전달 사항
1. **UI 참고**: 디자인 시안의 'Walk' 버튼은 Wake(기상) 모드를 의미합니다.
2. **이미지 참고**: 시안상의 터치 포인트(보라색 원)는 인터랙션 예시일 뿐 실제 구현할 필요는 없습니다.
3. **날짜 처리**: 계산 결과가 자정을 넘어가는 경우(다음 날/전날) 날짜 처리를 정확히 해주세요.
